<template>
  <CollapseTransition easing="ease-in-out">
    <n-card title="播放列表" closable class="playlistModel" v-show="music.showPlayList && music.getPlaylists.length"
      :header-style="{
        padding: '12px 16px',
        fontSize: '16px',
  ...
  flexDirection: 'column',
}" @close="music.showPlayList = false" @click.stop>
      <n-scrollbar>
    ...
    alignItems: 'center',
          ...
          <div class="right">
            ...
            <n-icon class="remove" :component="RemoveCircleOutlineFilled" @click.stop="music.removeSong(index)" />
          </div>
        </n-card>
      </n-scrollbar>
    </n-card>
