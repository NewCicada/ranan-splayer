<template>
  <div class="videolists">
    <Transition mode="out-in">
      <n-grid id="list" x-gap="20" y-gap="26" cols="2 s:3 m:4 l:5" responsive="screen" v-if="listData[0]">
        <n-gi class="item" v-for="item in listData" :key="item" @click="router.push(`/video?id=${item.id}`)">
          <div class="cover">
            <n-avatar class="coverImg" lazy :intersection-observer-options="{
              root: '#list',
      ...
      <n-grid v-else class="loading" x-gap="20" y-gap="26" cols="2 s:3 m:4 l:5" responsive="screen">
