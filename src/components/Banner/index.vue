<template>
  <n-skeleton v-if="!bannerData[0]" :style="{ height: bannerHeight + 'px' }" width="100%" :sharp="false" />
  <Transition>
    <n-carousel autoplay draggable keyboard class="banner" :effect="bannerType" :style="{ height: bannerHeight + 'px' }"
      v-if="bannerData[0]">
      <n-carousel-item class="item" :style="bannerType == 'card' ? 'width:60%' : ''" v-for="item in bannerData"
        :key="item">
        <img :src="item.imageUrl.replace(/^http:/, 'https:') + '?imageView&quality=89'
          " alt="banner" @click="bannerJump(item.targetType, item.targetId, item.url)" />
      </n-carousel-item>
    </n-carousel>
  </Transition>
</template>
