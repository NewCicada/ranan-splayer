<template>
  <div class="papersonalfm" v-if="music.getPersonalFmData.id" :style="'background-image: url(' +
    ...
    <div class="data">
      <div class="info">
        <span class="name text-hidden" @click="router.push(`/song?id=${music.getPersonalFmData.id}`)">{{
          music.getPersonalFmData.name }}</span>
        <AllArtists class="text-hidden" :artistsData="music.getPersonalFmData.artist" />
      </div>
      <div class="controls">
        <n-icon class="state" size="46" :component="music.getPersonalFmMode
          ? music.getPlayState
            ? PauseCircleFilled
            : PlayCircleFilled
          : PlayCircleFilled
          " @click="fmPlayOrPause" />
           ...
           <n-icon class="dislike" size="20" :component="ThumbDownRound"
