<template>
  <div class="playlists">
    <div class="menu">
      <!-- 分类选择 -->
      <n-button strong secondary class="cat" icon-placement="right" round @click="catModelShow = true">
        <template #icon>
          <n-icon class="up" :component="ChevronRightRound" />
        </template>
        {{ catName }}
      </n-button>
      <n-modal class="cat-model" v-model:show="catModelShow" style="width: 60vw; min-width: min(24rem, 100vw)"
        preset="card" title="歌单分类" :bordered="false">
        <template #header>
          歌单分类
          <n-tag round class="tag" :type="catName == '全部歌单' ? 'primary' : 'default'" :style="{
            marginLeft: '12px',
            fontSize: '12px',
            transform: 'translateY(-2px)',
          }" :bordered="false" @click="changeTagName('全部歌单')">
            全部歌单
          </n-tag>
        </template>
          ...
          <div class="all-cat" v-if="music.catList.sub && music.catList.sub[0]">
            <n-list>
              <n-list-item v-for="(cat, key) in music.catList.categories" :key="cat">
                ...
                <n-space>
                  <n-tag round class="tag" size="large" v-for="item in music.catList.sub.filter(
                    (v) => v.category == key
                  )" :key="item" :bordered="false" :type="item.name == catName ? 'primary' : 'default'"
      ...
      <!-- 精品歌单开关 -->
      <n-space v-if="getHaveHqPlaylists(music.highqualityCatList, catName)" align="center">
        <n-text>精品歌单</n-text>
        <n-switch v-model:value="hqPLayListOpen" @update:value="hqPLayListChange" :round="false" />
      </n-space>
    </div>
    <CoverLists :listData="playlistsData" />
    <Pagination v-if="playlistsData[0] && !hqPLayListOpen" :totalCount="totalCount" @pageSizeChange="pageSizeChange"
      @pageNumberChange="pageNumberChange" />
    <n-space justify="center">
      <n-button v-if="hqPLayListOpen && hasMore" class="more" size="large" strong secondary round :loading="loading"
        @click="() => {
            loading = true;
