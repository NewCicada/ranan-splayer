    ...
    <Transition mode="out-in">
      <n-grid class="official" x-gap="20" y-gap="20" :cols="2" v-if="toplistData.officialList[0]">
        <n-gi v-for="item in  toplistData.officialList " :key="item">
          <n-card class="item" :bordered="false" :content-style="{
            padding: '0',
            display: 'flex',
            alignItems: 'center',
          }" hoverable @click="router.push(`/playlist?id=${item.id}`)">
            <n-avatar class="cover" :src="item.coverImgUrl.replace(/^http:/, 'https:') + '?param=300y300'
