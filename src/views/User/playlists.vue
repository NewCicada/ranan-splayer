<template>
  <div class="playlists">
    <div class="data">
      <n-button class="add" type="primary" strong secondary round @click="createModelShow = true">
        <template #icon>
          <n-icon :component="AddCircleRound" />
        </template>
        新建歌单
      </n-button>
      <n-modal class="create" v-model:show="createModelShow" style="width: 60vw; min-width: min(24rem, 100vw)"
        preset="card" title="新建歌单" :bordered="false" :on-after-leave="createClose">
        <n-input style="margin-bottom: 12px" v-model:value="createName" type="text" placeholder="请输入新歌单标题" />
        ...
        <template #footer>
          <n-space justify="end">
            ...
            <n-button type="primary" @click="createPlaylistBtn(createName, createPrivacy)">
              新建
            </n-button>
          </n-space>
        </template>
